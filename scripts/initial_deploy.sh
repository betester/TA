...
gcloud compute instances create-with-container "$kafka_service_name" \
    ...
    --container-mount-host-path=host-path=/tmp,mode=ro,mount-path=/bitnami \
...
./create_local_instance.sh "$master_service_name" "$project" "$zone" "$service_account" "$master_envs"

# Running the master component
./run_service.sh  "master" "$master_service_name" "$zone" "$github_repository"
